    ...
    span.addEventListener("click", () => {
      ...
      if (menuItem.bankName) {
        ...
        if (bank) {
          ...
          if (menuItem.type !== "content") {
            const bankHeader = bank.bankLabel || bank.name;
            const bankContent = bank.bankContent || "";

            display.innerHTML = `
              <h1>${bankHeader}</h1>
              <p class="pad-content">${bankContent}</p>
            `;
          }
        }
      }
...
function buildTabs(data) {
  ...
  data.categories.forEach((category, index) => {
    ...
    tab.addEventListener("click", () => {
      ...
      const bankHeader = category.bankLabel || category.name;
      const bankContent = category.bankContent || "";
